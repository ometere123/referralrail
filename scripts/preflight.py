...
def main():
    ...
    markers=[("candidate explicit acceptance","def accept_referral" in settle),("inconclusive recovery","def retry_inconclusive" in settle and "def recover" in settle),("finalized judge message",'emit(on="finalized").evaluate' in settle),("finalized judgment with pull resolution",'emit(on="finalized").evaluate' in settle and "def resolve_judgment" in settle),("substantive validator rerun","independent = evaluate_once" in judge and "run_nondet(" in judge),("source restriction","https://api.github.com/repos/" in judge),("v2 identity contract","class ReferralIdentityV2" in identity and "def complete_github" in identity and "def complete_x" in identity),("v2 evidence profiles","GITHUB_PR" in v2 and "PUBLIC_WEB" in v2 and "PUBLIC_WEB" in v2j and "participated" in v2),("v2 campaign capacity","class ReferralRailV2" in v2 and "max_positions" in v2),("v2 referral-link self-join","def join_via_referral" in v2 and "referrer_address" in v2),("v2 finalized child judgment",'emit(on="finalized").evaluate' in v2),("v2 capacity settlement invariant","pending_successes" in v2 and "_used_capacity" in v2),("v2 cancel only unused campaigns", "def cancel_campaign" in v2 and "int(c.successful) != 0" in v2 and "int(c.paid_total) != 0" in v2 and "initial_funding" in v2),("v2 source restricted judge","https://api.github.com/repos/" in v2j)]
    for label,match in markers: good &= match; (ok if match else fail)(label)
    for label,match in [("v2 has no active X_POST", "X_POST" not in v2 and "X_POST" not in v2j), ("v2 has no active PUBLIC_URL", "PUBLIC_URL" not in v2 and "PUBLIC_URL" not in v2j), ("v2 frontend validation module", "validateV2Campaign" in (ROOT/"frontend/lib/v2Validation.ts").read_text() and "validPublicEvidenceUrl" in (ROOT/"frontend/lib/v2Validation.ts").read_text()), ("v2 dynamic fee estimation", "estimateTransactionFeesForWrite" in (ROOT/"integrations/sdk/src/fees.ts").read_text()), ("v2 durable SDK readback", "durable v2 readback" in (ROOT/"integrations/sdk/src/v2.ts").read_text()), ("v2 MCP public evidence wording", "public HTTPS evidence URL" in (ROOT/"integrations/mcp/src/server.ts").read_text())]: good &= match; (ok if match else fail)(label)
    ...
    if vm.exists():
        try:
            d=json.loads(vm.read_text()); sdk_v2=(ROOT/"integrations/sdk/src/constants.ts").read_text(); checks=[d.get("chainId")==EXPECTED_CHAIN,d.get("rpc")==EXPECTED_RPC,ADDR.match(str((d.get("rail") or {}).get("address",""))),ADDR.match(str((d.get("judge") or {}).get("address",""))),str((d.get("rail") or {}).get("address","")) in sdk_v2,str((d.get("judge") or {}).get("address","")) in sdk_v2]
