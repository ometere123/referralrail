...
const candidate2 = createAccount(`0x${"12".repeat(32)}`);
...
if (!campaign) {
  create = await write(employer, "create_campaign", [title, "Verify multiple candidates with fresh GitHub evidence and deterministic escrow settlement.", "The pull request must be authored by the bound GitHub identity and contain substantive work.", "ometere123", "evifix", "main", 2, reward, reward, 900, 1800, 7200, 2], reward * 4n);
...
const accept2 = rows[1].state === "ACCEPTED" ? null : await write(candidate2Client, "accept_referral", [campaignId, p2, "dependabot"]);
