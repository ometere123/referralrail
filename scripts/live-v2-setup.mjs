...
const createArgs = (repo, prBranch = "main") => [process.env.V2_TITLE || "Live v2 verification", "Complete the requested repository work and submit a qualifying pull request.", "The pull request must be authored by the bound GitHub identity and contain substantive work.", repo.split("/")[0], repo.split("/")[1], prBranch, 1, reward, reward, 900, 1800, 3600, 1];
...
const success = await campaign(process.env.V2_REPO || "ometere123/evifix");
...
const accept = await write(candidate,"accept_referral",[success.id,pid,process.env.V2_GITHUB_LOGIN || "ometere123"]);
...
writeFileSync(process.env.V2_SETUP_OUTPUT || "deployment/v2-live-setup.json",JSON.stringify(out,null,2)+"\n");
