...
V2 evidence is frozen to the campaign repository, base branch, GitHub login, acceptance timestamp, pull request identity, and ownership challenge. Do not relax freshness or proof checks to turn an old or inconclusive pull request into a success. Retry only through `retry_inconclusive` within the bounded cure window. Recover only after the contract's timeout or exhausted-cure rule allows it.
