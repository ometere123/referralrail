...
ReferralRail is a funded referral protocol. The employer creates and funds a multi-position campaign, a third-party referrer attributes a candidate, and the candidate joins, explicitly accepts, and submits bounded evidence. The deployed OutcomeJudge evaluates structured GitHub PR or universal public web evidence through GenLayer consensus. It is not an off-chain employment decision or an agent-controlled judge.
...
V2 evidence is profile-aware. GITHUB_PR freezes the repository, base branch, identity handle, acceptance timestamp, pull request identity, and ownership challenge. PUBLIC_WEB freezes the HTTPS host policy, submitted URL, acceptance timestamp, and challenge; it does not require GitHub or X identity. X, Medium, and similar sites are frontend presets that map to PUBLIC_WEB. The current web API cannot prove a final redirect destination. Do not relax freshness or proof checks to turn stale or inconclusive evidence into a success. Retry only through `retry_inconclusive` within the bounded cure window. Recover only after the contract's timeout or exhausted-cure rule allows it.

Position challenges bind submitted evidence to the exact campaign position. They are not cryptographic random nonces and do not prove page creation after acceptance; PUBLIC_WEB challenges are especially limited because no identity digest is present. Preserve that limitation in user-facing explanations.
