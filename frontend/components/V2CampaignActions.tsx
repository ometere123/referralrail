...
export default function V2CampaignActions({ campaign, positions }: Props) {
  ...
  return <section className="panel panel-pad" style={{ marginTop: 18 }}>
    ...
    {same(wallet.address, campaign.employer) && campaign.state === "ACTIVE" && campaign.occupied === 0 && campaign.successful === 0 && campaign.paid_total === 0 && <Write campaign={campaign} method="cancel_campaign" args={[campaign.id]} label="Cancel unused campaign and refund escrow" verify={(next) => next.state === "CANCELLED"} />}
