...
function TxBox({method,args,verify,label,onDone}:{method:string;args:any[];verify:()=>Promise<boolean>;label:string;onDone:()=>Promise<void>}){
 ...
 return <div><div className="eyebrow">{label}</div><VerifiedTransaction tx={tx} verify={verify} onVerified={onDone}/></div>;
}
...
export default function OpportunityPage(){
 ...
 return <div className="container detail-shell">
  <div className="detail-top"><div><Link href="/" className="meta">← All opportunities</Link><div className="eyebrow" style={{marginTop:17}}>Opportunity #{o.id} · {o.repo_owner}/{o.repo_name}</div><h1>{o.title}</h1><div className="detail-state"><span className={`status ${o.state.toLowerCase()}`}>{o.state}</span><span className="meta">last outcome {o.last_outcome}</span></div></div><a className="button secondary small" href={`${EXPLORER_URL.replace(/\/$/,'')}/contracts/${settlementAddress()}`} target="_blank" rel="noreferrer">Contract <ExternalLink size={13}/></a></div>
  <Lifecycle state={o.state}/>
  {o.state==='PAID'&&<div className="settlement-highlight"><div><small>Candidate paid</small><strong>{gen(o.candidate_payment)}</strong><span className="meta">{shortAddress(o.candidate)}</span></div><div><small>Referrer earned because the accepted referral completed</small><strong>{gen(o.referral_reward)}</strong><span className="meta">{shortAddress(o.referrer)}</span></div></div>}
  ...
  <div className="panel action-card">
   ...
   {!wallet.connected?<div className="notice warning">Connect a wallet to see the action available to that address.</div>:
    ...
    action?<div><button className="button secondary small" onClick={()=>setAction('')} style={{marginBottom:12}}>← Back</button>
      ...
      {action==='cancel'&&<TxBox method="cancel_unreferred" args={[id]} label="Cancel before any referral is locked" verify={()=>verifyState(x=>x.state==='CANCELLED')} onDone={refresh}/>} 
      {action==='expire'&&<TxBox method="expire" args={[id]} label="Apply deterministic expiry" verify={()=>verifyState(x=>x.state==='EXPIRED')} onDone={refresh}/>} 
      {action==='recover'&&<TxBox method="recover" args={[id]} label="Recover funds from bounded stalled path" verify={()=>verifyState(x=>x.state==='REFUNDED')} onDone={refresh}/>} 
    </div>:
    <div>
      ...
      {o.state==='JUDGING'&&<div className="notice info"><Hourglass size={15}/> GenLayer is resolving the GitHub evidence. The settlement callback is emitted only when the judge transaction finalizes.</div>}
      ...
      {['PAID','REFUNDED','EXPIRED','CANCELLED'].includes(o.state)&&<div className="notice info">This opportunity is terminal. No settlement action can execute again.</div>}
    </div>}
  </div>
 </div>
}
