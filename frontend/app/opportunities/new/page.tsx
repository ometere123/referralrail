...
export default function NewOpportunity(){
 ...
 const total=cp+rr;
 const tx=useMemo<SubmitInput>(()=>({kind:'write',address:settlementAddress() as `0x${string}`,method:'create_opportunity',args:[form.title.trim(),form.brief.trim(),form.criteria.trim(),form.owner.trim(),form.repo.trim(),form.candidate.trim(),cp,rr,BigInt(Number(form.referralHours||0)*3600),BigInt(Number(form.completionHours||0)*3600)],value:total} as SubmitInput),[form,cp,rr,total]);
 ...
 const verify=async()=>{const rows=await listOpportunities();return rows.some(o=>o.title===form.title.trim()&&o.candidate.toLowerCase()===form.candidate.toLowerCase()&&o.employer.toLowerCase()===wallet.address?.toLowerCase()&&o.repo_owner===form.owner.trim()&&o.repo_name===form.repo.trim()&&BigInt(o.funded_amount as any)===total)};
