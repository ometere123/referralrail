...
const address = z.string().regex(/^0x[0-9a-fA-F]{40}$/).refine((value) => !/^0x0{40}$/i.test(value), "zero address is not allowed");
const amount = z.string().regex(/^[0-9]+$/).transform(BigInt);
const state = (raw: unknown) => JSON.parse(JSON.stringify(raw, (_key, value) => typeof value === "bigint" ? value.toString() : value));
...
export function createReferralRailServer(): McpServer {
  const server = new McpServer({ name: "referralrail", version: "0.1.0" });
  const { client, account, writeEnabled } = configuredClient();
  ...
  if (process.env.REFERRALRAIL_V2_ADDRESS && process.env.OUTCOMEJUDGE_V2_ADDRESS) {
    ...
    server.registerTool("referralrail_v2_get_position", { description: "Read one finalized v2 position.", inputSchema: { campaignId: id, positionId: id } }, async ({ campaignId, positionId }) => text(await v2.client.getPosition(campaignId, positionId)));
    server.registerTool("referralrail_v2_get_accounting", { description: "Read finalized v2 campaign accounting and conservation values.", inputSchema: { campaignId: id } }, async ({ campaignId }) => text(await v2.client.getCampaignAccounting(campaignId)));
    ...
    server.registerTool("referralrail_v2_get_available_actions", { description: "Determine profile-aware legal v2 actions from finalized campaign and position state.", inputSchema: { campaignId: id, positionId: id.optional(), address: address.optional() } }, async ({ campaignId, positionId, address: actor }) => text(await v2.client.getAvailableActions(campaignId, positionId, actor as `0x${string}` | undefined)));
    ...
    v2write("referralrail_v2_accept_referral", "Accept a v2 referral. GitHub campaigns bind the identity handle; public web campaigns require no platform identity.", { campaignId: id, positionId: id, identityHandle: z.string().optional().default("") }, x => v2.client.acceptReferral(x.campaignId, x.positionId, x.identityHandle));
    v2write("referralrail_v2_decline_referral", "Candidate declines a reserved v2 position.", { campaignId: id, positionId: id }, x => v2.client.declineReferral(x.campaignId, x.positionId));
    v2write("referralrail_v2_release_referral", "Referrer releases a reserved v2 position.", { campaignId: id, positionId: id }, x => v2.client.releaseReferral(x.campaignId, x.positionId));
    v2write("referralrail_v2_submit_work", "Submit a GitHub pull request number for v2 consensus judgment.", { campaignId: id, positionId: id, prNumber: id }, x => v2.client.submitWork(x.campaignId, x.positionId, x.prNumber));
    v2write("referralrail_v2_submit_evidence", "Submit a public HTTPS evidence URL for v2 consensus judgment.", { campaignId: id, positionId: id, evidenceUri: z.string().url().refine((value) => value.startsWith("https://"), "HTTPS URL required") }, x => v2.client.submitEvidence(x.campaignId, x.positionId, x.evidenceUri));
    v2write("referralrail_v2_retry_inconclusive", "Retry an inconclusive v2 judgment with profile-specific replacement evidence.", { campaignId: id, positionId: id, submission: z.string().min(1) }, x => v2.client.retryInconclusive(x.campaignId, x.positionId, x.submission));
    ...
    v2write("referralrail_v2_close_intake", "Close v2 campaign intake and begin final refund resolution.", { campaignId: id }, x => v2.client.closeIntake(x.campaignId));
    v2write("referralrail_v2_finalise_campaign", "Finalize v2 campaign and refund unused backing.", { campaignId: id }, x => v2.client.finaliseCampaign(x.campaignId));
    v2write("referralrail_v2_cancel_campaign", "Cancel an unused active v2 campaign and refund its escrow.", { campaignId: id }, x => v2.client.cancelCampaign(x.campaignId));
  }
  return server;
}
